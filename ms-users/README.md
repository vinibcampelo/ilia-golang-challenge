...
- Go (versão conforme `go.mod` / `toolchain` do módulo).
...
O `Dockerfile` neste diretório compila o binário estático e copia o `ms-users.yaml` para uso com `OPENAPI_SPEC` no container. O `docker-compose.yml` define as variáveis necessárias para subir o serviço junto ao Postgres.
